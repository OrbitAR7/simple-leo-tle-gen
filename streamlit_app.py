...
def get_random_parameters():
    ...
    altitude = random.randint(400, 2000)
...
def main():
    ...
    # App header
    st.set_page_config(
        page_title="LEO TLE Constellation Generator",
        page_icon="🛰️",
        layout="wide",
        initial_sidebar_state="expanded"
    )

    st.title("🛰️ LEO TLE Constellation Generator")
    ...
    # Add validation warning for Walker F
    total_sats_preview = num_planes * sats_per_plane
    if walker_F > total_sats_preview:
        st.sidebar.warning(f"⚠️ Walker F ({walker_F}) is larger than total satellites ({total_sats_preview}). This may cause unexpected phasing.")
    ...
    # Information section
    with st.sidebar.expander("About Walker Constellations"):
        st.markdown("""
    ...
    if hasattr(st.session_state, 'generate_constellation') and st.session_state.generate_constellation:
        ...
        designed to be compatible with **NCAT.site** and other orbital analysis software.
